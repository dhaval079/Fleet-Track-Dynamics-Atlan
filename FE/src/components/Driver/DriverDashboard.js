...
const DriverDashboard = () => {
  ...
  const fetchDriverInfo = async () => {
    try {
      const response = await fetch(`http://atlan-load-balancer-1090299753.ap-south-1.elb.amazonaws.com/api/v2/drivers/${driverId}`, {
        headers: {
...
const barChartOptions = {
  ...
  const toggleAvailability = async () => {
    try {
      const response = await fetch(`http://atlan-load-balancer-1090299753.ap-south-1.elb.amazonaws.com/api/v2/drivers/${driverId}/availability`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const updateJobStatus = async (jobId, newStatus) => {
    try {
      const response = await fetch(`http://atlan-load-balancer-1090299753.ap-south-1.elb.amazonaws.com/api/v2/drivers/jobs/${jobId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
